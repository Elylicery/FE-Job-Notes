...
<img src="D:\Users\Documents\[job]求职攻略\前端技术面试笔记.assets\image-20201109204935432.png" alt="image-20201109204935432" style="zoom:50%;" />
...
* **inline-block、inline和block的区别**
  ...
  * *Inline-block*：能设置宽度高度，margin/padding水平垂直方向都有效，左后又可以有其他元素
...
* BFC是一个==作用范围==，是一块==独立渲染区域==，内部元素的渲染不会影响边界以外的元素
* 形成BFC的常见条件
  1. float不是none
  2. position是absolute或fixed
  3. overflow不是visible
  4. display是flex inline-block inline-flex等
* BFC的常见应用

  * 清除浮动：当元素设置了*overflow样式且值不为visible时，该元素就构建了一个BFC，BFC在计算高度时，内部浮动元素的高度也要计算在内，也就是说即使BFC区域内只有一个浮动元素时，BFC的高度也不会发生塌缩，所以达到了清除浮动的目的，*
* 阻止margin发生重叠
  * 阻止元素被浮动的元素覆盖
...
答案就在 BFC 这个区域的==独立==特性上。因为独立，所以它要确保自己的作用范围不会对外界产生影响。此处我们把父元素创建为了一个 BFC，那么父元素就要保证自己的子元素不会跑出去给别人添麻烦，所以***会自动把自己的宽高适应到能囊括子元素的程度。***
...
给两个子元素各创建一个父元素，然后设置`overflow:hidden`让这个父元素是一个BFC。

**BFC应用3——阻止元素被浮动的元素覆盖**

应用在两列布局，一列宽度固定，另一列自适应中。
...
<img src="img/image-20201210212047823.png" alt="image-20201210212047823" style="zoom:25%;" />
...
**题目1：为什么设置auto可以让元素相对于父元素水平居中？**

答：auto这个属性的取值，只有两种：1. 父元素剩余空间的宽度 2. 0

当元素布局方式为static/relative且宽高已知时，auto取1中的值；当元素的布局方式为position/abosulte/fixed或者宽高未知时，auto取2中的值。（上述只针对于水平方向）

**题目2：如何利用auto实现元素的垂直居中？**

答：利用元素的==流体特性==：当一个绝对定位元素，其对立定位方向属性同时有具体定位数值的时候，流体特性就发生了：**元素可自动填充父级元素的可用尺寸。**

当流体特性发生时，我们可以给水平/垂直方向的对立定位（也就是left，right，top，bottom）各设定一个值，然后将水平/垂直方向的margin均设为auto，这样以来，auto就会自动平分父元素的剩余空间了。
...
**题目1：对于不知道子元素宽高的情况下，结合定位和transform一起使用**
...
**题目2：vertical-align:middle 什么时候能让元素垂直居中？**

答：`vertical-align`只有在display为inline和table-cell时起作用

补充：对于大小不固定的元素垂直居中：父元素设置：`display:table`; 子元素：`display:table-cell; vertical-align:middle;`
...
也叫视口，在移动端有两种，布局视口（layout viewport 页面实际布局所占用的区域）与视觉接口（visual viewport）

```html
<meta name="viewport" content="width=device-width">
```

![image-20210222125400278](前端技术面试笔记.assets/image-20210222125400278.png)

* **window.innerWidth** 和 **window.innerHeight**获取**视觉视口的宽高**
* **document.documentElement.clientWidth** 获取**布局视口的宽度**（eg `docucment.body.clientHeight`就是body的高度）
* **window.screen.height** 指的是**屏幕高度**

### 响应式布局的常见方案

响应式布局主要解决的是屏幕大小不确定的问题

<img src="前端技术面试笔记.assets/image-20210222154440897.png" alt="image-20210222154440897" style="zoom: 27%;" />

### 方案1 - 媒体查询

去感知屏幕大小的变化，根据不同的屏幕大小展示不同的样式。
...
rem是一个以根元素font-size大小为基准的相对单位，如果以rem作为布局单位，只要根元素大小发生改变，页面中所有相关元素的大小都会进行相应的放缩。如果我们能够根据设备屏幕的大小，动态地修改根元素的font-size，就相当于间接地修改了页面中所有元素的大小，进而实现了响应式布局。

<img src="前端技术面试笔记.assets/image-20210222144436856.png" alt="image-20210222144436856" style="zoom: 67%;" />

![image-20210222144542927](前端技术面试笔记.assets/image-20210222144542927.png)

**单位区别：**

* **px**，绝对长度单位，最常用
* **em**：相对长度单位，相对的是使用他们的元素（当前元素）的字体大小
  * em在一些情况下会刚好取到父元素的字体大小，但这是由继承导致的
* **rem**：相对长度单位，相对于HTML根元素，常用于响应式布局
...
![image-20210222144824095](前端技术面试笔记.assets/image-20210222144824095.png)
...
```
window.devcePixelRatio = 设备的物理像素/css像素
```

如果devicePixelRatio为2，意味着我设置的1pxCSS像素，在这个设备上实际会用2个物理像素单元来进行渲染，所以实际看到的一定会比1px粗一些。

**思路一：直接写0.5px**

![image-20210220220208380](前端技术面试笔记.assets/image-20210220220208380.png)

**思路二：伪元素先放大后缩小 **

![image-20210220220448888](前端技术面试笔记.assets/image-20210220220448888.png)

<img src="前端技术面试笔记.assets/image-20210220220536429.png" alt="image-20210220220536429" style="zoom:50%;" />

**思路三：viewport缩放来解决**

```html
<meta name="viewport" content="initial-scale=0.5,maximun-sacle=0.5,mininum-scale=0.5,user-scalable=no">
```

这里针对像素比为2的页面，把整个页面缩放为原来的1/2。根据像素比的不同，这个缩放比例可以被计算为不同的值，js实现如下：

```js
const scale = 1/window.devicePixelRatio;
metaEl.setAttribute('content',`width=device-with,user-sacleable=no, initial-sacle=${scale},maximun-sacle=${scale},mininum-scale=${scale}5`)
```

副作用：整个页面被缩放了。解决：==高清方案==
...
<img src="前端技术面试笔记.assets/image-20210220223929359.png" alt="image-20210220223929359" style="zoom:33%;" />
...
PostCss是一个对CSS进行解析和处理的工具，类似于JS世界里的Babel。PostCss：可以编译尚未被浏览器广泛支持的先进的CSS语法。

<img src="前端技术面试笔记.assets/image-20210220224254879.png" alt="image-20210220224254879" style="zoom:33%;" />

### （四）webpack能处理CSS吗？

1. webpack在裸奔的情况下不能处理CSS：因为webpack本身就是一个面向js且只能处理js代码的模块化打包工具
2. webpack在loader的辅助下，是可以处理CSS的

如何用webpack实现对css的处理：两个关键的loader：css-loader和style-loader
...
![image-20210206113851619](前端技术面试笔记.assets\image-20210206113851619.png)
...
* `Object.create({...})`可指定原型
* `Object.create(obj)`创建了一个空对象，其原型指向了传入的对象
...
* instanceof 可以判断引用类型(Function Array Object)
* Object是所有class的父类
...
该释放的变量（内存垃圾）没有被释放，仍然霸占着原先的内存，导致内存占用攀高，带来性能恶化，系统崩溃等一系列问题

**闭包对内存的”威胁“？**

举个问题代码的例子：

<img src="前端技术面试笔记.assets/image-20210225214457681.png" alt="image-20210225214457681" style="zoom: 67%;" />
...
![image-20210130170917956](前端技术面试笔记.assets\image-20210130170917956-1612581826489.png)

**2.全局代码中的this**

全局代码中的this，就是在全局作用域下执行的函数/代码段里的this

![image-20210130171143118](前端技术面试笔记.assets\image-20210130171143118-1612581826489.png)
...
![image-20210130194401904](前端技术面试笔记.assets\image-20210130194401904-1612581902006.png)
...
<img src="D:\Users\Documents\[job]求职攻略\前端技术面试笔记.assets\image-20210209120250335.png" alt="image-20210209120250335" style="zoom: 33%;" />

Node整体上由这三部分组成

* **应用层**：Nodejs代码，包括Node应用以及一些标准库
* **桥阶层**：Node底层由C++实现。桥阶层负责封装底层依赖的C++模块的能力，将其简化为API向应用层提供服务
* **底层依赖**：最最底层的C++库，支持Node运行的最基本能力
  * V8是JS的运行引擎，它把js代码转换为c++，然后去跑c++代码
  * **libuv**：**它对跨平台的异步I/O能力封装，Node中的事件循环就是由libuv来初始化的**；

#### Node中的Event loop和浏览器的有啥区别？

>  浏览器的EventLoop由各个浏览器自己实，Node的Event loop由libuv来实现

### libuv中的Event-Loop实现

libuv主导循环机制共有六个循环阶段

<img src="前端技术面试笔记.assets/image-20210209122426128.png" alt="image-20210209122426128" style="zoom:50%;" />

这六个阶段各是处理什么任务

* **timers阶段**：执行setTimeout和setInterval中定义的回调
* **pending callbacks**：对于网络IO或文件IO过程中出现的错误，在这个阶段处理错误的回调
* **idle，prepare**：系统内部使用（我们不用操心）
* **poll（轮询阶段）**：重点阶段，这个阶段会执行I/O回调，会检查定时器是否到期
* **check（检测阶段）**：处理setImmediate中定义的回调
* **close callbacks**：处理一些关闭的回调

**Node中的事件循环流程**

1. 执行全局Scirpt代码
2. 把微任务队列清空
   * 浏览器中，只有一个微任务对象需要接受处理
   * 在Node中由两类：next-tick队列和其他队列
   * **清空队列时，优先清空next-tick队列中的任务，随后清空其他微任务**
3. 开始执行宏任务
4. 步骤3开始，会进入3->2->3->2...的循环

### nextTick和Promise.then的关系

> Node 清空微任务队列的手法比较特别。在浏览器中，我们只有一个微任务队列需要接受处理；但在 Node 中，有两类微任务队列：next-tick 队列和其它队列。其中这个 **next-tick 队列**，专门用来收敛 process.nextTick 派发的异步任务。**在清空队列时，优先清空next-tick队列中的任务，随后才会清空其他微任务。**
...
User-Agent是一个特征字符串，用来标识发起请求的用户代理软件的应用类型、操作系统、软件开发商和版本号的信息
...
<img src="前端技术面试笔记.assets\image-20210206191808197.png" alt="image-20210206191808197" style="zoom: 33%;" />
...
<img src="前端技术面试笔记.assets\image-20210206195024866.png" alt="image-20210206195024866" style="zoom: 50%;" />
...
* 同源：协议，域名，端口，三者必须一致
* 同源策略：ajax请求时，出于安全考虑，不同源的脚本在没有明确授权的情况下，不能读写对方的资源
...
由于js调用跨域文件是被允许的（`<script>`可绕过跨域限制），只要我们在远程服务器上设法把数据装进js格式的文件里，就可以供客户端调用。

<img src="img/image-20201126093747150.png" alt="image-20201126093747150" style="zoom:67%;" />

JSONP就是允许用户传递一个callback参数给服务端，然后服务端返回数据时会将这个callback参数作为函数名来包裹住JSON数据，这样客户端就可以随意定制自己的函数来自动处理返回数据了。

![image-20210220204417559](前端技术面试笔记.assets/image-20210220204417559.png)

![image-20210220204504829](前端技术面试笔记.assets/image-20210220204504829.png)
...
注意：对于开发者来说，CROS通信与通原来的通信没有差别，代码上一样。浏览器一旦发现AJAX请求跨域，就会自动添加一些**附加的头信息、追加必要的请求**，但用户不会有感觉。

![image-20201126094319398](img/image-20201126094319398.png)

**CROS和JSONP的对比**

JSONP只支持GET请求，而CROS支持所有类型的HTTP请求。但CROS的兼容性没有JSONP好。
...
> 2. TCP提供可靠的服务。也就是说，通过TCP连接传送的数据，无差错，不丢失，不重复，且按序到达;UDP尽最大努力交付，即不保证可靠交付。 并且因为tcp可靠，面向连接，不会丢失数据因此适合大数据量的交换。（TCP适合，像文件传入这样对可靠性和稳定性要求比较高、同时连接比较确定的场合）
> 3. TCP是面向字节流；UDP面向报文，并且网络出现拥塞不会使得发送速率降低（对实时性要求比较高的场合，例如网络电视、视频会议、在线直播等，UDP合适）。
> 4. TCP只能是1对1的，UDP支持1对1,1对多，（*UDP可以面向多方提供服务*）
> 5. TCP的首部较大为20字节，而UDP只有8字节。*（同样的报文内容，UDP比TCP更高效）
...
<img src="前端技术面试笔记.assets/image-20210220162424044.png" alt="image-20210220162424044" style="zoom: 33%;" />

![image-20201211220908849](../[Interview]牛客前端工程师面试宝典/牛客前端面经.assets/image-20201211220908849.png)

**三次握手过程**

1. 发送端发送一个带SYN标志的数据包给对方
   * SYN报文：起标识作用，当SYN=1且ACK=0时，表明这是一个连接请求报文
2. 接收端收到后，回传一个带有SYN/ACK标志的数据包以示传达确认信息
   * 接收端若同意进行连接，则在响应报文中使SYN=1且ACK=1，因此，SYN置1就表示这是一个连接请求或连接接受报文
3. 最后，发送端再回传一个带ACK标志的数据包，代表握手结束
   * ACK报文：TCP协议规定，只有当ACK=1时有效，也规定连接建立后所有发送的报文的ACK都为1

**题目1：为什么TCP一定要握手三次?**

<img src="前端技术面试笔记.assets/image-20210220163155038.png" alt="image-20210220163155038" style="zoom: 50%;" />

**题目2：为什么要四次挥手？**

<img src="前端技术面试笔记.assets/image-20210220162604550.png" alt="image-20210220162604550" style="zoom:50%;" />

<img src="前端技术面试笔记.assets/image-20210220162646146.png" alt="image-20210220162646146" style="zoom: 33%;" />

<img src="前端技术面试笔记.assets/image-20210220162727853.png" alt="image-20210220162727853" style="zoom: 50%;" />
...
![image-20210220172552735](前端技术面试笔记.assets/image-20210220172552735.png)

![image-20210220173537211](前端技术面试笔记.assets/image-20210220173537211.png)

**题目2：HTTP发展史**

**HTTP1.0的问题**

* *连接不可复用*：每进行一次通信，都有走三步—— TCP连接、HTTP通信、断开TCP连接，我们希望的是 TCP连接、HTTP通信1、HTTP通信2、HTTP通信3....断开TCP连接（即在一个TCP连接里可以进行多次HTTP通信）
* *对头阻塞问题*：请求和请求间是串行的，B请求必须等待A请求的响应返回才可以发出。

**==HTTP1.1== 使用管线化解决问题**

![image-20210220184028729](前端技术面试笔记.assets/image-20210220184028729.png)

**==HTTP2.0== 改进性能**

* **二进制分帧**：在HTTP1.x中数据以文本的格式进行传输，解析起来比较低效。HTTP2.0在传输消息时，首先将消息划分为更小的消息和帧，然后对其进行二进制格式的编码，确保高效的解析。
* **头部压缩**：客户端和服务端分别维护一份相同的静态字典，这个字典用来存储常见头部名称，以及常见的名称和值的组合
* **服务端推送**：服务端可以将依赖资源等推送给客户端
* **多路复用**：一次连接建立后，只要这个连接还在，客户端就可以在一个连接中批量发起多个请求。同时，请求与请求间完全不阻塞。
...
* HTTPS是**身披SSL外壳的Http**
  ...
  * 通常，HTTP直接和TCP通信。当使用SSL，则演变成先和SSL通信，再由SSL和TCP通信了是
* HTTPS**采用混合加密机制**
  * Https采用共享密钥加密和公共密钥加密两者并用的混合加密机制（也就是对称加密+非对称加密混合使用）
    * 对称加密：客户端和服务器端都约定一个共同的“公钥”，加密和解密都依赖于这一个公钥。
    * 非对称加密：服务器保留一个私钥，并且把这个私钥关联的公钥发给客户端，客户端用这个公钥对数据加密，服务端收到数据后用私钥解密（公钥和私钥多对一，公钥加密的内容只有私钥可以解开，私钥加密的内容，所有公钥都可以解开）
  * Https协议需要CA证书，费用较高
...
![image-20210220171014226](前端技术面试笔记.assets/image-20210220171014226.png)
...
1. 安装Babel `npm install @babel/core @babel/preset-env babel-loader-D`三个插件
   ...
   <img src="img/image-20201129155807666.png" alt="image-20201129155807666" style="zoom: 67%;" />
...
<img src="前端技术面试笔记.assets\image-20210206204431928.png" alt="image-20210206204431928" style="zoom:33%;" />
...
![](.\前端技术面试笔记.assets\无标题-1612783533549.png)

**题目1：为何建议把CSS放在head中？**

* 在DOM树加载完之前就得到CSS的规则，DOM树生成后就与所有的CSS整合得到一个Render树，一步渲染完成，不用重复
* 如果DOM生成后才加载css，如果网速慢或者浏览器很老，可能会出现DOM树加载中，字体变大变小这样的视觉错乱

**题目2：为何建议把js放在body最后**

* 如果没有放到body最后，可能出现页面渲染一部分卡住了，加载完js后又继续渲染的情况（延长了页面渲染的时间）
* js放在最后，不阻碍页面的渲染，使得DOM渲染完之后再执行js

## 16-3 重绘和重排

> 备注：用于盲答”针对*页面渲染流程中的性能优化*，你有什么思路？“

重走页面声明周期的过程有两种，重排与重绘

### 重排

当操作引发了**DOM几何尺寸的变化**（比如修改元素的宽、高或隐藏元素时）等，浏览器需要重新计算元素的几何属性（其他元素的几何属性和位置也会因此受到影响），然后再将计算的结果绘制出来，这个过程就是**重排**（也叫回流）

> 重排多数情况由对元素的几何属性的修改引起
>
> 其他情况：
>
> * 改变DOM树结构：节点的增减、移动等
> * 获取一些特定属性的值（重要）如offsetWidth，scrollWidth，clientWidth等需要通过**即时计算**得到的属性。因此浏览器为了获取这些值，也会进行重排

<img src="D:\Users\Documents\[job]求职攻略\前端技术面试笔记.assets\image-20210209113441828.png" alt="image-20210209113441828" style="zoom:50%;" />

### 重绘

对DOM的修改导致了**样式的变化**，却并未影响其几何属性（比如修改了颜色或背景色）时，浏览器不需要重新计算元素的几何属性、直接为该元素绘制新的样式（跳过了上述的重排环节），这个过程叫重绘

> 重绘是对元素绘制属性的修改引发的。

<img src="D:\Users\Documents\[job]求职攻略\前端技术面试笔记.assets\image-20210209113906282.png" alt="image-20210209113906282" style="zoom: 50%;" />

**有哪些方法可以减少重绘和重排？**

* *尽可能小的影响重排的范围*：尽可能在低层级的DOM节点上，而不是像上述全局范围的示例代码一样，如果你要改变p的样式，class就不要加在div上，通过父元素去影响子元素不好。

* *样式集中改变*：使用`csstext`,`className`一次性改变属性
...
* *分离DOM的读写操作*：读操作放在一起，写操作放在一起

* *使用fragment*：通过 [documentFragment](https://developer.mozilla.org/zh-CN/docs/Web/API/DocumentFragment) 创建一个 `dom` 碎片,在它上面批量操作 `dom`，操作完成之后，再添加到文档中，这样只会触发一次重排。

* *使用 absolute 或 fixed 脱离文档流*:

  * 使用绝对定位会使的该元素单独成为渲染树中 `body` 的一个子元素，重排开销比较小，不会对其它节点造成太多影响。
  * 所以对于多次重排的元素，比如说动画。使用绝对定位脱离文档流，使其不影响其他元素
...
* XSRF(跨站请求伪造 Cross-Site Request Forgeries)
